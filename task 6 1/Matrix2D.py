...
class Matrix2D:
    def __init__(self, *a):
        if len(a) == 1:
            if isinstance(a[0], Matrix2D):
                ...
                self.a11, self.a12, self.a21, self.a22 = map(float, a[0])
            else:
                if len(a[0]) != 2 or len(a[0][0]) != 2 or len(a[0][1]) != 2:
                    raise ValueError("Неправильна структура матриці")

                self.a11, self.a12 = map(float, a[0][0])
                self.a21, self.a22 = map(float, a[0][1])
            ...
            self.a11, self.a12, self.a21, self.a22 = map(float, a)

        elif len(a) == 0:  # одинична матриця
            self.a11, self.a12, self.a21, self.a22 = 1.0, 0.0, 0.0, 1.0
    ...
    def __str__(self):
        return f"{self.a11:.2f} {self.a12:.2f}\n{self.a21:.2f} {self.a22:.2f}"
