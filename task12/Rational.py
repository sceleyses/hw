from RationalError import RationalnError
from RationalValueError import RationalValueError
...
class Rational:
    def __init__(self, numerator, denominator=1):
        if isinstance(numerator, str):
            ...
            if len(parts) != 2:
                raise RationalValueError(numerator, denominator, "Неприпустимий формат рядка!")

            self.n = int(parts[0])
            self.d = int(parts[1])
            if denominator != 1:
                raise RationalValueError(numerator, denominator)
        else:
            if not isinstance(numerator, int) or not isinstance(denominator, int):
                raise RationalValueError(numerator, denominator, "Чисельник та знаменник повинні бути цілими числами!")

            self.n = numerator
            self.d = denominator

        if self.d == 0:
            raise RationalnError
